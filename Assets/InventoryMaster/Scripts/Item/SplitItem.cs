...
{     //splitting an Item
    ...
    public void OnPointerDown(PointerEventData data)                    //splitting the item now
    {
        ...
        if (data.button == PointerEventData.InputButton.Left && pressingButtonToSplit && inv.stackable && (inv.getItemList().Count < (inv.height * inv.width))) //if you press leftclick and and keycode
        {
